...
export function buildSafetyReview(stats) {
  const flags = []
  const issues = stats?.issueCountMap || {}

  if ((stats?.redCount || 0) > 0) {
    flags.push({ level: 'red', code: 'red_event', text: '本周存在红色事件。优先停止相关实践并按安全边界处理；本工具不会在这种情况下推荐加量或高级练法。' })
  }
  ...
  if ((stats?.yellowCount || 0) >= 2) {
    flags.push({ level: 'yellow', code: 'repeated_yellow', text: '本周黄色事件重复出现。优先暂停触发问题的单元、缩短或回退，而不是增加时长。' })
  }
  ...
  if ((stats?.overLimitCount || 0) >= 2) {
    flags.push({ level: 'yellow', code: 'repeated_over_limit', text: '最近7天有多次实际时长超过当前卡片审查上限。记录应保留，但下一步优先回到已审查负荷，而不是把超时常态化。' })
  }

  if (!stats?.recordCount) {
    flags.push({ level: 'info', code: 'empty', text: '最近7天还没有记录。可以从一次短时、低负荷的基础实践开始，也可以选择今天不练。' })
  } else if (!flags.length) {
    flags.push({ level: 'info', code: 'stable', text: '最近7天没有触发重复安全规则。保持当前低负荷即可，不需要因为“状态不错”自动增加时长。' })
  }
